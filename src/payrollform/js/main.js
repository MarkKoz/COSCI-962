...
function getHours() {
    ...
    const inputs = Array.from(form.wts.getElementsByTagName("input"));

    // Sums all the values of the inputs. Initial value of 0 so it starts at
    // 0th index isntead of 1st. If the value can't be parsed (e.g. when input
    // is empty), adds 0 instead, effectively ignoring it.
    return inputs.reduce((acc, val) => acc + (parseInt(val.value) || 0), 0);
}

function getTaxRate() {
    const select = form.panel;

    return select.options[select.selectedIndex].value / 100;
}
